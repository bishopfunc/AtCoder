...
struct Clock {
  ...
  string to_str() {
    ...
    if (hour < 10) str_h = "0" + str_h; 
    else if (hour == 24) str_h = "00";
    if (minute < 10) str_m = "0" + str_m; 
    if (second < 10) str_s = "0" + str_s; 
    return str_h + ":" + str_m + ":" + str_s;
  }

  void shift(int diff_second) {
    int now_sec, now_min, now_hour;
    now_sec = hour * 60 * 60 + minute * 60 + second;
    if (hour == 0 && diff_second < 0) now_sec += 24 * 60 * 60;
    now_sec += diff_second;
